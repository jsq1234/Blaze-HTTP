...
int bz_add_event(int epfd, data_t *d, int flags) {
	...
	if (flags & BZ_READABLE)
		e.events |= EPOLLIN;
	if (flags & BZ_WRITEABLE)
		e.events |= EPOLLOUT;
	if (flags & BZ_EDGE_TRIG)
		e.events |= EPOLLET;
	...
	if (nfds < 0) {
		...
		for (int i = 0; i < nfds; i++) {
			...
			if (event.events & EPOLLIN) {
				...
				if (d->filefd < 0) {
					/*
						A negative filefd indicates that the data_t belongs to
					   the server. Thus, we have a new connection
					*/
	...
	for (;;) {
		...
		if (bytesRcv == 0) {
			/*
				...
				If we still have any data that we want to send, we should send
			   it before closing our end.
			*/
...
void bz_handle_new_connection(event_loop_t *event_loop, data_t *d) {
	...
	for (;;) {
		...
		if (!conn) {
			fprintf(stderr, "Couldn't allocate memory for conn\n");
			close(connfd);
			continue;
		}
